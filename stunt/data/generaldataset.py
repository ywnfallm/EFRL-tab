...
#             if self.source == 'val':
#
#                 num_way = num_way if num_way <= len(class_list) else len(class_list)
...
class GeneralDataset(object):
    def __init__(self, dataset, tabular_size, seed, source, shot, tasks_per_batch, test_num_way, query):
        super().__init__()
        self.num_classes = 2
        self.tabular_size = tabular_size
        self.source = source
        ...
        self.query = query
        self.tasks_per_batch = tasks_per_batch
        self.unlabeled_x = np.load(f'./data/{dataset}/train_x.npy')
        self.test_x = np.load(f'./data/{dataset}/xtest.npy')
        self.test_y = np.load(f'./data/{dataset}/ytest.npy')
        self.val_x = np.load(f'./data/{dataset}/val_x.npy')
        self.val_y = np.load(f'./data/{dataset}/val_y.npy')  # val_y is given from pseudo-validaiton scheme with STUNT
        self.test_num_way = test_num_way
        self.test_rng = np.random.RandomState(seed)
        self.val_rng = np.random.RandomState(seed)
    ...
    def get_batch(self):
        ...
        for _ in range(self.tasks_per_batch):
            ...
            if self.source == 'val':
                ...
                for k in classes:
                    ...
                    k_idx = k_idx[permutation]
                    support_idx.append(k_idx[:num_val_shot])
                    query_idx.append(k_idx[num_val_shot:num_val_shot + self.query]) # + 30
                    if len(k_idx) < num_val_shot + self.query:
                        bu = num_val_shot + self.query - len(k_idx)
                        for _ in range(bu):
                            query_idx.append(k_idx[-1:])
                support_idx = np.concatenate(support_idx)
                query_idx = np.concatenate(query_idx)
                ...
                min_count = 0
                while min_count < (self.shot + self.query):
                    min_col = int(x.shape[1] * 0.4) # 0.2
                    max_col = int(x.shape[1] * 0.7) # 0.5
                    col = np.random.choice(range(min_col, max_col), 1, replace=False)[0]
                    task_idx = np.random.choice([i for i in range(x.shape[1])], col, replace=False)
                    masked_x = np.ascontiguousarray(x[:, task_idx], dtype=np.float32)
                    kmeans = faiss.Kmeans(masked_x.shape[1], num_way, niter=20, nredo=1, verbose=False,
                ...
                for k in classes:
                    ...
                    support_idx.append(k_idx[:self.shot])
                    query_idx.append(k_idx[self.shot:self.shot + self.query])
                    if len(k_idx) < self.shot + self.query:
                        bu = self.shot + self.query - len(k_idx)
                        for _ in range(bu):
                            query_idx.append(k_idx[-1:])
                support_idx = np.concatenate(support_idx)
                query_idx = np.concatenate(query_idx)
        ...
        if self.source == 'val':
            xq = np.reshape(
                xq,
                [self.tasks_per_batch, num_way * self.query, self.tabular_size]) # * 30
        else:
            xq = np.reshape(
                xq,
                [self.tasks_per_batch, num_way * self.query, self.tabular_size])
