    ...
    def forward(self, x):
        ...
        for feat_init_idx, is_continuous in enumerate(self.continuous_idx):
            ...
            if is_continuous:
                ...
                # 第一种方法：每个嵌入都随机在其小领域内扰动, noise 系数分别为 0.1 和 0.01
                # noise = (torch.rand(len(x), self.cat_emb_dims[cat_feat_counter])-0.5)*0.01
                # noise = noise.to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
                # cat_embeddings = self.embeddings[cat_feat_counter](x[:, feat_init_idx].long())
                # cat_embeddings = cat_embeddings + noise if self.training else cat_embeddings
                # cols.append(cat_embeddings)
                # cat_feat_counter += 1

                # 第二种方法：每个嵌入都添加高斯噪声，均值 0, 标准差 1 / 100cat_emb_dim 和 1 / 1000cat_emb_dim
                noises = []
                for i in range(len(x)):
                    noise = torch.randn(1, self.cat_emb_dims[cat_feat_counter]) / (1000.0 * self.cat_emb_dims[cat_feat_counter])
                    noises.append(noise)
